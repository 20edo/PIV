...
def saveVectorField(image_number, save=0, minus_average=0, fill='noise'):
        ...
        frame_a = rank.equalize(frame_a, selem = selem, mask=(np.logical_not(flashMask_a)))

        frame_b = frame_b.astype('uint8')
        frame_b = rank.equalize(frame_b, selem = selem, mask=(np.logical_not(flashMask_b)))
        ...
        winsize = 24 # pixels, interrogation window size in frame A
